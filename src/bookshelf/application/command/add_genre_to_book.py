...
class AddGenreToBookCommand:
    book_id: str
    genre_name: str


class AddGenreToBookHandler:
    def __init__(self, book_repository: BookRepository) -> None:
        self._book_repository = book_repository

    async def __call__(self, command: AddGenreToBookCommand) -> None:
        book = await self._book_repository.find_by_id(BookId(command.book_id))
        if book is None:
            raise BookNotFoundError(command.book_id)

        book.add_genre(Genre(command.genre_name))
